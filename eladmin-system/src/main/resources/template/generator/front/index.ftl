  ...
  <div class="app-container">
    ...
    <#if hasQuery>
      <div v-if="crud.props.searchToggle">
        ...
        <el-input v-model="query.${column.changeColumnName}" clearable placeholder="<#if column.remark != ''>${column.remark}<#else>${column.changeColumnName}</#if>" style="width: 134px;" class="filter-item" @keyup.enter.native="crud.toQuery" />
            </#if>
          </#list>
        </#if>
  <#if betweens??>
    <#list betweens as column>
      ...
      <el-dialog :close-on-click-modal="false" :before-close="crud.cancelCU" :visible.sync="crud.status.cu > 0" :title="crud.status.title" :width="dialogStyle">
        <el-form ref="form" :inline="true" :model="form" <#if isNotNullColumns??>:rules="rules"</#if> size="small" label-width="80px">
    <#if columns??>
      <#list columns as column>
        <#if column.formShow>
          <el-form-item label="<#if column.remark != ''>${column.remark}<#else>${column.changeColumnName}</#if>"<#if column.istNotNull> prop="${column.changeColumnName}"</#if>>
            ...
            <el-input v-model="form.${column.changeColumnName}" style="width: 194px;" />
            <#elseif column.formType = 'Textarea'>
            <br>
            <el-input v-model="form.${column.changeColumnName}" :rows="3" type="textarea" :style="'width:'+textareaStyle" />
            <br>
            ...
            <#elseif column.formType = 'Select'>
              <#if (column.dictName)?? && (column.dictName)!="">
            <el-select v-model="form.${column.changeColumnName}" placeholder="请选择" style="width: 194px">
              <el-option
            ...
            <#else>
            <el-date-picker v-model="form.${column.changeColumnName}" type="datetime" style="width: 194px;" />
            </#if>
          </el-form-item>
        </#if>
      </#list>
      ...
      <el-table ref="table" v-loading="crud.loading" :data="crud.data" size="small" style="width: 100%;" @selection-change="crud.selectionChangeHandler">
        ...
        <#if columns??>
          ...
          <#if (column.dictName)?? && (column.dictName)!="">
        <el-table-column prop="${column.changeColumnName}" label="<#if column.remark != ''>${column.remark}<#else>${column.changeColumnName}</#if>" align="center">
